...
## 2026-01-19 - XSS in Torrent Titles
**Vulnerability:** Torrent titles from magnet links were passed directly to the API response without sanitization. An attacker could craft a magnet link with a malicious `dn` parameter (e.g., containing `<script>`) to execute XSS on the client.
**Learning:** Never trust data from external sources, even if it comes from a "structured" format like a magnet link. Also, existing security libraries (like `htmlSanitize` using `DOMPurify`) might be broken or ineffective in the server environment, creating a false sense of security.
**Prevention:** Always escape or sanitize user-controllable data before outputting it. When using security libraries, verify they actually work in your specific environment (e.g. Node vs Browser) via tests. Using simpler, environment-agnostic solutions like `he.encode` can be more robust for plain text fields.
- 2026-03-01: [HIGH] Unreliable server-side `dompurify` HTML sanitization leading to potential XSS; replaced with `he.encode` for robust plain text entity escaping.
