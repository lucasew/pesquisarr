...
- 2026-07-03: When refactoring security functions like htmlSanitize, ensure you check the memory correctly. The instruction 'Server-side HTML sanitization using `dompurify` combined with `linkedom` is unreliable in this project's environment and should be avoided. Use `he.encode` for escaping HTML entities for XSS protection instead.' is explicitly provided in the memory context, overriding the code review's assumption that `he.encode` is functionally incorrect.
